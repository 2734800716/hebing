...
*/
    ...
    form.on('submit(menuUser)',function(data){
		...
		$.post("/KaoShi_1/QxianInsert?mname="+mname+"&mfunction="+url+"&type2="+type2+"&mbtn="+mbtn+"&father="+father,function(data){
			var info = eval("("+data+")");
	 		if(info == 1){
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("/KaoShi_1/BtnSelect",function (res) {
        		var cs = eval('(' + res + ')');
        		var btntype = $('#btnType');
        ...
        $.get("/KaoShi_1/QxType?type="+type,function (res) {
        	var cs = eval('(' + res + ')');
            var data = cs.data;
            var dom = $("#father");
            if(type == 1){
